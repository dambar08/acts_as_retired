...
module ActsAsRetired
  ...
  # inverse is a belongs_to association with the :with_retired option set.
  #
  # Specifying :with_retired for the belongs_to association would stop the
  ...
  # scope was added just to support the :with_retired option.
  module AssociationReflection
    if ActiveRecord::VERSION::MAJOR < 7
      def can_find_inverse_of_automatically?(reflection)
        options = reflection.options

        if reflection.macro == :belongs_to && options[:with_retired]
      ...
      def scope_allows_automatic_inverse_of?(reflection, inverse_reflection)
        if reflection.scope
          options = reflection.options
          return true if options[:with_retired] && !options.fetch(:original_scope)
        end
